<x-app-layout>
  ...
  <div class="flex flex-col sm:flex-row sm:justify-between items-center mb-8 gap-4">
    <h1 class="text-2xl font-bold tracking-tight text-gray-800 dark:text-gray-100">Categorias</h1>
    ...
    <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl">
      <thead>
        <tr class="text-white" style="background-color: #991b1b !important;">
          ...
          <th class="px-4 py-3 text-left font-semibold">Máximo de Horas</th>
      ...
      <tbody>
        ...
        <tr class="hover:bg-red-50 dark:hover:bg-gray-800 transition">
          ...
          <td class="px-4 py-3">{{ $categoria->maximo_horas }}</td>
          <td class="px-4 py-3">{{ $categoria->curso->nome ?? '-' }}</td>
          <td class="px-6 py-3 whitespace-nowrap">
            <div class="flex flex-wrap gap-2">
              <a href="{{ route('coordenador.categorias.show', $categoria->id) }}"
              ...
              <form action="{{ route('coordenador.categorias.destroy', $categoria->id) }}" method="POST"
                class="inline-block" onsubmit="return confirm('Tem certeza que deseja excluir esta categoria?');">
                @csrf
                @method('DELETE')
        ...
        @empty
        <tr>
          <td colspan="5" class="text-center text-gray-500 dark:text-gray-400 py-8">Nenhuma categoria cadastrada.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
