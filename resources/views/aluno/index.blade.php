<x-app-layout>
  ...
  <div class="flex flex-col sm:flex-row sm:justify-between items-center mb-8 gap-4">
    <h1 class="text-2xl font-bold tracking-tight text-gray-800 dark:text-gray-100">Alunos</h1>
    <a href="{{ route('coordenador.alunos.create') }}"
      class="btn from-red-700 to-red-500 hover:from-red-800 hover:to-red-600 shadow-lg">
  ...
  <div class="overflow-x-auto rounded-xl shadow">
    <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl">
      ...
      <tbody>
        ...
        <tr class="hover:bg-red-50 dark:hover:bg-gray-800 transition max-w-full">
          <td class="px-4 py-3">{{ $aluno->id }}</td>
          <td class="px-4 py-3">{{ $aluno->nome }}</td>
          <td class="px-4 py-3">{{ $aluno->cpf }}</td>
          <td class="px-4 py-3">{{ $aluno->email }}</td>
          <td class="px-4 py-3">{{ $aluno->curso->nome }}</td>
          <td class="px-4 py-3">{{ $aluno->curso->sigla }} {{ $aluno->turma->ano }}</td>
          <td class="px-6 py-3 whitespace-nowrap ">
            <div class="flex flex-wrap gap-2 max-w-full">
              <a href="{{ route('coordenador.alunos.show', $aluno->id) }}"
                class="btn bg-indigo-600 hover:bg-indigo-700 flex items-center gap-1">
              ...
              <form action="{{ route('coordenador.alunos.destroy', $aluno->id) }}" method="POST" class="inline-block"
        ...
        @empty
        <tr>
          <td colspan="7" class="text-center text-gray-500 dark:text-gray-400 py-8">Nenhum aluno cadastrado.
          </td>
        </tr>
        @endforelse
      </tbody>
