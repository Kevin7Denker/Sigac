<x-app-layout>
  ...
  <div class="flex flex-col sm:flex-row sm:justify-between items-center mb-8 gap-4">
    <h1 class="text-2xl font-bold tracking-tight text-gray-800 dark:text-gray-100">Eixos</h1>
    <a href="{{ route('coordenador.eixos.create') }}"
    ...
    <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl">
      ...
      <tbody>
        ...
        <tr class="hover:bg-red-50 dark:hover:bg-gray-800 transition">
          <td class="px-4 py-3">{{ $eixo->id }}</td>
          <td class="px-4 py-3">{{ $eixo->nome }}</td>
          <td class="px-6 py-3 whitespace-nowrap">
            <div class="flex flex-wrap gap-2">
              <a href="{{ route('coordenador.eixos.show', $eixo->id) }}"
              ...
              <form action="{{ route('coordenador.eixos.destroy', $eixo->id) }}" method="POST" class="inline-block"
        ...
        @empty
        <tr>
          <td colspan="3" class="text-center text-gray-500 dark:text-gray-400 py-8">Nenhum eixo cadastrado.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
