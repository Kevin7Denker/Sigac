<x-app-layout>
  ...
  <div class="flex flex-col sm:flex-row sm:justify-between items-center mb-8 gap-4">
    <h1 class="text-2xl font-bold tracking-tight text-gray-800 dark:text-gray-100">Cursos</h1>
    <a href="{{ route('coordenador.cursos.create') }}"
  ...
  <div class="overflow-x-auto rounded-xl shadow">
    <table class="min-w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl">
      <thead>
        <tr class="text-white" style="background-color: #991b1b !important;"></tr>
        <th class="px-4 py-3 text-left font-semibold">ID</th>
        <th class="px-4 py-3 text-left font-semibold">Nome</th>
        <th class="px-4 py-3 text-left font-semibold">Sigla</th>
      ...
      <tbody>
        ...
        <tr class="hover:bg-red-50 dark:hover:bg-gray-800 transition">
          <td class="px-4 py-3">{{ $curso->id }}</td>
          <td class="px-4 py-3">{{ $curso->nome }}</td>
          <td class="px-4 py-3">{{ $curso->sigla }}</td>
          <td class="px-6 py-3 whitespace-nowrap">
            <div class="flex flex-wrap gap-2">
              <a href="{{ route('coordenador.cursos.show', $curso->id) }}"
              ...
              <form action="{{ route('coordenador.cursos.destroy', $curso->id) }}" method="POST" class="inline-block"
        ...
        @empty
        <tr>
          <td colspan="4" class="text-center text-gray-500 dark:text-gray-400 py-8">Nenhum curso cadastrado.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
